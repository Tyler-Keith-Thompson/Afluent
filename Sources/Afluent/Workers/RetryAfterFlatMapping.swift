...
extension AsynchronousUnitOfWork {
    ...
    public func retry<D: AsynchronousUnitOfWork>(_ strategy: some RetryStrategy, @_inheritActorContext @_implicitSelfCapture _ transform: @Sendable @escaping (Error) async throws -> D) -> some AsynchronousUnitOfWork<Success> {
    ...
    ///
    /// - Returns: An `AsynchronousUnitOfWork` that emits the same output as the upstream but retries on the specified error up to the specified number of times, with the applied transformation.
    public func retry<D: AsynchronousUnitOfWork, E: Error & Equatable, S: RetryStrategy>(_ strategy: S, on error: E, @_inheritActorContext @_implicitSelfCapture _ transform: @Sendable @escaping (E) async throws -> D) -> some AsynchronousUnitOfWork<Success> {
        Workers.RetryOnAfterFlatMapping(upstream: self, strategy: strategy, error: error, transform: transform)
    }
}
