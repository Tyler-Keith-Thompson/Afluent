...
final class ThrottleSequenceTests: XCTestCase {
    ...
    func testThrottleOnlyReturnsFirstAndLatestElementImmediately_whenReceivingMultipleElementsAtOnceAndStreamEnds_andLatestIsTrue() async throws {
        await withMainSerialExecutor {
            let testClock = TestClock()
            let elementContainer = ElementContainer()
    ...
    func testThrottleOnlyReturnsFirstAndSecondElementImmediately_whenReceivingMultipleElementsAtOnceAndStreamEnds_andLatestIsFalse() async throws {
        await withMainSerialExecutor {
            let testClock = TestClock()
            let elementContainer = ElementContainer()
    ...
    func testThrottleOnlyThrottlesAfterFirstElementIsReceived_whenLatestIsTrue() async {
        await withMainSerialExecutor {
            ...
            let stream = AsyncThrowingStream { continuation in
                ...
                .delayThenHandleOutput(for: .milliseconds(10)) { _ in
        ...
        func testThrottleOnlyReturnsLatestElement_whenMultipleElementsAreReceivedAfterMultipleThrottleIntervalsHavePassed_10ms_andLatestIsTrue() async throws {
            await withMainSerialExecutor {
                ...
                let stream = AsyncStream { continuation in
                    ...
                    .delayThenHandleOutput(for: .milliseconds(100)) { _ in
                        continuation.yield(2)
                        continuation.yield(3)
                        continuation.yield(4)
                        continuation.yield(6)
    ...
    //            let stream = AsyncStream { continuation in
    //                DeferredTask { () }
    //                    .delay(for: .milliseconds(10)) { _ in
    //                        continuation.yield(1)
    //                    }
    //                    .delay(for: .milliseconds(50)) { _ in
    //                        continuation.yield(2)
    //                        continuation.yield(3)
    //                        continuation.yield(4)
    //                        continuation.yield(6)
    ...
    //                        continuation.yield(4)
    //                    }
    //                    .delay(for: .milliseconds(100)) { _ in
    //                        continuation.yield(5)
    //                    }
    //                    .delay(for: .milliseconds(10)) { _ in
    //                        continuation.yield(6)
